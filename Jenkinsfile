pipeline {
    ...
    stages {
        ...
        stage('Generate Version') {
            steps {
                script {
                    // Fetch the version generated by the Semantic Versioning Plugin
                    def semverVersion = readFile('target/semver.txt').trim()
                    echo "Generated Semantic Version: ${semverVersion}"
            ...
            steps {
                script {
                    ...
                    withCredentials([usernamePassword(credentialsId: 'dockerhub', passwordVariable: 'DOCKERHUB_PASSWORD', usernameVariable: 'DOCKERHUB_USERNAME')]) {
                        def dockerRepo = "sebastine/project-tsukinome-${params.BRANCH_NAME}:${env.VERSION}"
                        ...
                        try {
                            ...
                            // Build Docker 
                            echo "Building application with version: ${env.VERSION}"
                            ...
                            sh "docker rmi ${registry}:${env.VERSION} || true"
                        } catch (Exception e) {
    ...
    post {
        always {
            script {
                ...
                slackSend channel: 'jenkinscicd', color: color, message: "*${buildResult}:* Job ${env.JOB_NAME} build ${env.env.VERSION} \n More info at: ${env.BUILD_URL}"
            }
        }
    }
}